...
async def api_predict(
    ...
    try:
        ...
        # Instantiate confluency model
        try:
            model = MSCConfluency(magnification=magnification, checkpoint=checkpoint_path, device=device)
...
def run_training_thread(
    ...
    try:
        ...
        training_state.add_log(f"Backbone: {encoder_backbone} | Learning Rate: {lr} | Batch Size: {batch_size}")
        ...
        try:
            ...
            training_state.set_finished(False, f"Failed to load data directories: {str(e)}")
            return
            
        if not train_pairs:
            training_state.set_finished(False, "No training pairs found. Verify train folder structure.")
            return
        if not val_pairs:
            training_state.set_finished(False, "No validation pairs found. Verify validation folder structure.")
            return
            
        training_state.add_log(f"Found {len(train_pairs)} training and {len(val_pairs)} validation pairs.")
        ...
        for epoch in range(1, epochs + 1):
            ...
            for batch_idx, (imgs, masks) in enumerate(train_loader):
                ...
                batch_progress = (epoch - 1) / epochs + (batch_idx / len(train_loader)) / epochs
            ...
            if avg_val_loss < best_val_loss:
                ...
                training_state.add_log(f"Saved new best checkpoint to: {checkpoint_path}")
...
async def api_train_start(
    ...
    if not os.path.isdir(train_dir):
        raise HTTPException(status_code=400, detail=f"Train directory does not exist: {train_dir}")
    if not os.path.isdir(val_dir):
        raise HTTPException(status_code=400, detail=f"Validation directory does not exist: {val_dir}")
    ...
    t = threading.Thread(
        ...
        args=(train_dir, val_dir, output_dir, epochs, lr, batch_size, encoder_backbone, seed),
        daemon=True,
    )
    t.start()
